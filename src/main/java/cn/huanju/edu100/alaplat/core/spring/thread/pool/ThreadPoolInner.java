...
public abstract class ThreadPoolInner {
    ...
    protected ReentrantReadWriteLock mainLock = new ReentrantReadWriteLock();
                ...
                for (ThreadPool pool : this.poolMap.values()) {
                    if (Objects.nonNull(pool)) {
                        if (Objects.nonNull(logger)) {
                            logger.info("[ThreadPoolScheduler] {}", pool.toString());
                        }
                    }
                }
            }
    ...
    private class ThreadPool {
                ...
                executor.lock();
                {
                    ...
                    if (Objects.nonNull(task)) {
                        ...
                        try {
                            ...
                            if (Objects.nonNull(logger)) {
                                logger.error("[ThreadPool] ThreadPool.handleTask(Task:{}) failed. {}", task.getName(), executor.toString(), e);
                            }
                        } finally {
        ...
        final class ThreadExecutor extends AbstractQueuedSynchronizer implements Runnable {
            ...
            final ReentrantReadWriteLock threadLock = new ReentrantReadWriteLock();
            BaseTask currTask;
